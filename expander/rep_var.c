...
/*   Updated: 2023/07/16 15:55:42 by rennacir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rep_var_help(t_list	*tmp, t_list **new_list, t_env *envir)
{
	...
	while (tmp)
	{
		...
		{
			ft_lstadd_back(new_list, ft_lstnew_tokens(ft_strdup(tmp->content), tmp->type));
			tmp = tmp->next;
		}
		...
		else
		{
			...
			if (node)
			{
				...
				while (split[i])
				{
					ft_lstadd_back(new_list, ft_lstnew_tokens(ft_strdup(split[i]), VARIABLE));
					if (split[i + 1])
